...
Ext.onReady(function() {
	...
	var deptTree = new Ext.tree.TreePanel({
				loader : new Ext.tree.TreeLoader({
							baseAttrs : {},
							dataUrl : _ctxpath + '/role/departmentTreeInit.do'
						}),
				root : root,
				title : '',
				applyTo : 'deptTreeDiv',
		...
		renderer : function(value, cellmeta, record) {
			if (login_account == parent.DEFAULT_DEVELOP_ACCOUNT
				...
				if (record.data['roletype'] == '1') {
					return '<img src="' + _ctxpath + '/resource/image/ext/edit2.png"/>';
				}
			}
			return '<a href="javascript:void(0);"><img src="' + _ctxpath + '/resource/image/ext/edit1.png"/></a>';
		},
		width : 35
	}, {
		header : '角色名称',
	...
	var store = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : _ctxpath + '/role/queryRolesForManage.do'
						}),
	...
	function saveRoleItem() {
		...
		addRoleFormPanel.form.submit({
					url : _ctxpath + '/role/saveRoleItem.do',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
				...
				.confirm(
						...
						function(btn, text) {
							if (btn == 'yes') {
								...
								showWaitMsg();
								Ext.Ajax.request({
									url : _ctxpath + '/role/deleteRoleItems.do',
	...
	function update() {
		addRoleFormPanel.form.submit({
					url : _ctxpath + '/role/updateRoleItem.do',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
	...
	function roleGrantInit(roleid, deptid, roletype) {
		...
		var operatorTab = new Ext.Panel({
			title : '<img src="' + _ctxpath + '/resource/image/ext/config.png" align="top" class="IEPNG"> 经办权限授权',
			// iconCls: 'user_femaleIcon',
			autoLoad : {
				url : _ctxpath + '/role/operatorTabInit.do',
				scripts : true,
		...
		var selectUserTab = new Ext.Panel({
			title : '<img src="' + _ctxpath + '/resource/image/ext/group.png" align="top" class="IEPNG"> 选择人员',
			// iconCls:'chart_organisationIcon',
			autoLoad : {
				url : _ctxpath + '/role/selectUserTabInit.do',
				scripts : true,
				text : parent.PAGE_LOAD_MSG,
				params : {
					roleid : roleid,
		...
		var managerTab = new Ext.Panel({
			title : '<img src="' + _ctxpath + '/resource/image/ext/wrench.png" align="top" class="IEPNG"> 授权权限授权',
			// iconCls: 'status_onlineIcon',
			autoLoad : {
				url : _ctxpath + '/role/managerTabInit.do',
				scripts : true,
				text : parent.PAGE_LOAD_MSG,
				params : {
					roleid : roleid,
