...
//				menu: themeMenu
	...
	themeTree.on('click', function(node) {
				...
				o.innerHTML = '<img src="' + _ctxpath + '/resource/image/theme/' + theme
						+ '.jpg" />';
			});
	...
	layoutTree.on('click', function(node) {
				...
				o.innerHTML = '<img src="' + _ctxpath + '/resource/image/theme/layout/' + layout
						+ '.jpg" />';
			});
	...
	function layoutWindowInit() {
		...
		var o = document.getElementById('previewDiv');
		o.innerHTML = '<img src="' + _ctxpath + '/resource/image/theme/layout/' + default_layout
				+ '.jpg" />';
		layoutWindow.show();
	...
	function saveUserLayout(o) {
		...
		showWaitMsg();
		Ext.Ajax.request({
			url : _ctxpath + '/index/saveUserLayout.do',
	...
	var lockWindow = new Ext.Window({
				...
				buttons : [{
							...
							handler : function() {
								window.location.href = _ctxpath + '/login/logout.do';
							}
						}]
			});
	...
	function unlockSystem() {
		...
		Ext.Ajax.request({
					url : _ctxpath + '/index/unlockSystem.do',
	...
	function themeWindowInit() {
		...
		var o = document.getElementById('previewDiv');
		o.innerHTML = '<img src="' + _ctxpath + '/resource/image/theme/' + default_theme + '.jpg" />';
		themeWindow.show();
	}
	...
	function saveUserTheme(o) {
		showWaitMsg();
		Ext.Ajax.request({
			url : _ctxpath + '/index/saveUserTheme.do',
			success : function(response) {
				var resultArray = Ext.util.JSON.decode(response.responseText);
				Ext.MessageBox
						.confirm(
		...
		userWindow.on('show', function() {
					setTimeout(function() {
								userFormPanel.form.load({
											...
											url : _ctxpath + '/index/loadUserInfo.do',
	...
	function updateUser() {
		...
		userFormPanel.form.submit({
					url : _ctxpath + '/index/updateUserInfo.do',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
