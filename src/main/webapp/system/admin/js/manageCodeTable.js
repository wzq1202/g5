...
Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : _ctxpath + '/resource/queryCodeItems.do'
						}),
			...
			handler : function() {
				...
				if (codeWindow.getComponent('codeForm').form.isValid()) {
					codeWindow.getComponent('codeForm').form.submit({
						url : _ctxpath + '/resource/saveCodeItem.do',
						waitTitle : '提示',
						method : 'POST',
						waitMsg : '正在处理数据,请稍候...',
	...
	function updateCodeItem() {
		...
		editCodeFormPanel.form.submit({
					url : _ctxpath + '/resource/updateCodeItem.do',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
	...
	function synMemory(msg, flag) {
		Ext.Msg.confirm('请确认', msg, function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
									url : _ctxpath + '/resource/synMemory.do',
	...
	function deleteCodeItems() {
		...
		Ext.Msg.confirm('请确认', '你真的要删除字典吗?', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
							Ext.Ajax.request({
										url : _ctxpath + '/resource/deleteCodeItem.do',
										success : function(response) {
											store.reload();
