...
Ext.onReady(function() {
	...
	var menuTree = new Ext.tree.TreePanel({
				loader : new Ext.tree.TreeLoader({
							baseAttrs : {},
							dataUrl : _ctxpath + '/resource/queryMenuItems.do'
						}),
				root : root,
				title : '',
				applyTo : 'menuTreeDiv',
	...
	var store = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : _ctxpath + '/resource/queryMenuItemsForManage.do'
						}),
	...
	function saveMenuItem() {
		...
		addMenuFormPanel.form.submit({
					url : _ctxpath + '/resource/saveMenuItem.do',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
	...
	function deleteMenuItems(pType, pMenuid) {
		...
		Ext.Msg.confirm('请确认', '你真的要删除选中菜单及其包含的所有子菜单吗?', function(btn, text) {
					if (btn == 'yes') {
						...
						showWaitMsg();
						Ext.Ajax.request({
									url : _ctxpath + '/resource/deleteMenuItems.do',
	...
	function update() {
		...
		var parentid_old = Ext.getCmp('parentid_old').getValue();
		addMenuFormPanel.form.submit({
					url : _ctxpath + '/resource/updateMenuItem.do',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
