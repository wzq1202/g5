	...
	<div id="x-desktop">
		...
		#foreach($menu in $menuList)
			<dt id="window_${menu.menuid}-shortcut">
				<a href="javascript:void(0);"><img src="${contextPath}/resource/image/ext/s.gif" />
							...
							if (!win) {
								win = desktop
										.createWindow({
											...
											constrain: true,
											html : '<iframe id="topMain" scrolling="${menu.scrollbar}" frameborder="0" style="width:100%;height:100%;" src="${contextPath}${menu.request}"></iframe>'
										});
							}
							win.show();
						}
...
/**************************/
	...
	function updateUser() {
		...
		userFormPanel.form.submit({
					url : '${contextPath}/index/updateUserInfo.do',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
		...
		userWindow.on('show', function() {
					setTimeout(function() {
								userFormPanel.form.load({
											...
											url : '${contextPath}/index/loadUserInfo.do',
											success : function(form, action) {
											},
											failure : function(form, action) {
												Ext.Msg
...
/*********************/
	...
	var lockWindow = new Ext.Window({
				...
				buttons : [{
							...
							handler : function() {
								window.location.href = '${contextPath}/login/logout.do';
							}
						}]
			});
	...
	function unlockSystem() {
		...
		Ext.Ajax.request({
					url : '${contextPath}/index/unlockSystem.do',
...
/**********************/
	...
	themeTree.on('click', function(node) {
				...
				o.innerHTML = '<img src="${contextPath}/resource/image/theme/' + theme
						+ '.jpg" />';
			});
	...
	function themeWindowInit() {
		...
		var o = document.getElementById('previewDiv');
		o.innerHTML = '<img src="${contextPath}/resource/image/theme/' + default_theme
				+ '.jpg" />';
		themeWindow.show();
	...
	function saveUserTheme(o) {
		showWaitMsg();
		Ext.Ajax.request({
			url : '${contextPath}/index/saveUserTheme.do',
	...
	layoutTree.on('click', function(node) {
				...
				o.innerHTML = '<img src="${contextPath}/resource/image/theme/layout/' + layout
						+ '.jpg" />';
			});
	...
	function layoutWindowInit() {
		...
		var o = document.getElementById('previewDiv');
		o.innerHTML = '<img src="${contextPath}/resource/image/theme/layout/' + default_layout
				+ '.jpg" />';
		layoutWindow.show();
	...
	function saveUserLayout(o) {
		showWaitMsg();
		Ext.Ajax.request({
			url : '${contextPath}/index/saveUserLayout.do',
			success : function(response) {
				var resultArray = Ext.util.JSON.decode(response.responseText);
				Ext.MessageBox
						.confirm(
	...
	backgroundTree.on('click', function(node) {
				...
				o.innerHTML = '<img src="${contextPath}/resource/desktop/wallpapers/mini/' + background + '" />';
			});
	...
	function backgroundWindowInit() {
		...
		var o = document.getElementById('previewDiv');
		o.innerHTML = '<img src="${contextPath}/resource/desktop/wallpapers/mini/' + default_background + '" />';
	...
	function saveUserbackground(o) {
		showWaitMsg();
		Ext.Ajax.request({
			url : '${contextPath}/index/saveUserBackground.do',
			success : function(response) {
				var resultArray = Ext.util.JSON.decode(response.responseText);
				Ext.MessageBox
						.confirm(
