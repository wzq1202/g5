...
Ext.onReady(function(){
            mainTabs = new Ext.TabPanel({
                ...
                items:[{
                      title:"<img align='top' class='IEPNG' src='${contextPath}/resource/image/ext/user.png'/>${welcomePageTitle}",
                      listeners: {activate: function(){Ext.getCmp('centerPanel').setTitle('${centerTitle} -> ${welcomePageTitle}');}},
                      html:"<iframe name='mainFrame'  src='${contextPath}/index/preferencesInit.do' scrolling='auto' frameborder='0' width='100%' height='100%' ></iframe>"
                    }]
             });
...
function addTab(url,name,menuid,pathCh,icon){
  ...
  if(index != -1){
  	if(url.substring(index+3,index+4) == '?'){
  		url = '${contextPath}' + url + '&menuid4Log=' + menuid;
  	}else{
  		url = '${contextPath}' + url + '?menuid4Log=' + menuid;
  	}
  }
  ...
  if (!n) {
     ...
     n = mainTabs.add({
       id:id,
       title:"<img align='top' class='IEPNG' src='${contextPath}/resource/image/ext/" + icon + "'/>" + name,
       closable:true,
       layout:'fit',
...
<DIV class=loading-indicator><IMG style="MARGIN-RIGHT: 8px"
	height=32
	src="${contextPath}/resource/image/ajax1.gif"
...
<tr>
<td width="65%"><nobr>&nbsp;<img class="IEPNG" src="${contextPath}/resource/image/ext/comments2.png" />&nbsp;欢迎您,${username}!&nbsp;帐户:${account}&nbsp;所属部门:${deptname}</nobr></td>
